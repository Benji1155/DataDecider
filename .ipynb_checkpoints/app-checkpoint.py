    ...
    for col in df.columns:
        try:
            ...
            if is_numeric_dtype(dtype):
                denominator = max(1, len(df)) # Should be non_null_count if using for ratio
                ...
                denominator = max(1, len(df)) # Should be non_null_count if using for ratio
                if unique_count <= 1: # Treat constant as categorical
            ...
            print(f"Warning: Type check failed for column '{col}': {e}")
            simplified_types[col] = 'other'
    return simplified_types
...
def suggest_charts_based_on_answers(user_answers, df_sample):
    ...
    if ("one" in ua_count or "1" in ua_count) and \
       ("dist" in ua_msg or "spread" in ua_msg or "summ" in ua_msg) and \
       ("num" in ua_types or "cat" in ua_types or "any" in ua_types or not ua_types) and \
       distributable_numeric_cols:
    ...
    if ("one" in ua_count or "1" in ua_count) and \
       ("prop" in ua_msg or "share" in ua_msg or "freq" in ua_msg or "count" in ua_msg or "val" in ua_msg) and \
       ("cat" in ua_types or "any" in ua_types or not ua_types) and \
       categorical_cols:
        for col in categorical_cols:
            ...
            try:
                if col in df_sample.columns: # Ensure column exists in sample
                ...
                print(f"Warning: Column '{col}' not found in sample for Pie Chart suggestion.")
            except Exception as e:
                print(f"Warning during pie chart suggestion for {col}: {e}")

    # Rule: TWO NUMERICAL variables (Relationship)
    if ("two" in ua_count or "2" in ua_count) and \
       ("relat" in ua_msg or "corr" in ua_msg or "scat" in ua_msg) and \
       ("num" in ua_types or "any" in ua_types or not ua_types) and \
       len(numerical_cols) >= 2:
        for i, c1 in enumerate(numerical_cols):
            for j, c2 in enumerate(numerical_cols):
                if j > i:
                    suggestions.append({"name": "Scatter Plot", "for_cols": f"{c1} & {c2}", "type": "Bivariate Numerical-Numerical", "reason": f"Relationship: '{c1}' vs '{c2}'.", "required_cols_specific": [c1, c2]})

    # Rule: ONE NUMERICAL/DISTRIBUTABLE vs ONE CATEGORICAL (Comparison)
    if ("two" in ua_count or "2" in ua_count) and \
       ("comp" in ua_msg or "across" in ua_msg or "group" in ua_msg or "dist" in ua_msg) and \
       ("mix" in ua_types or "cat" in ua_types or "num" in ua_types or "any" in ua_types or not ua_types) and \
       distributable_numeric_cols and categorical_cols:
        for num_col in distributable_numeric_cols:
            for cat_col in categorical_cols:
                if num_col != cat_col:
                    suggestions.append({"name": "Box Plots (by Category)", "for_cols": f"{num_col} by {cat_col}", "type": "Bivariate Numerical-Categorical", "reason": f"Distribution of '{num_col}' across '{cat_col}'.", "required_cols_specific": [cat_col, num_col]})
                    suggestions.append({"name": "Violin Plots (by Category)", "for_cols": f"{num_col} by {cat_col}", "type": "Bivariate Numerical-Categorical", "reason": f"Density/distribution of '{num_col}' across '{cat_col}'.", "required_cols_specific": [cat_col, num_col]})
        if numerical_cols and categorical_cols: # For aggregated bar chart specifically
            for num_col in numerical_cols:
                for cat_col in categorical_cols:
                    if num_col != cat_col:
                        suggestions.append({"name": "Bar Chart (Aggregated)", "for_cols": f"Avg of {num_col} by {cat_col}", "type": "Bivariate Numerical-Categorical", "reason": f"Average '{num_col}' for each category in '{cat_col}'.", "required_cols_specific": [cat_col, num_col]})

    # Rule: TWO CATEGORICAL variables
    if ("two" in ua_count or "2" in ua_count) and \
       ("relat" in ua_msg or "comp" in ua_msg or "cont" in ua_msg or "joint" in ua_msg) and \
       ("cat" in ua_types or "any" in ua_types or not ua_types) and \
       len(categorical_cols) >= 2:
        for i, c1 in enumerate(categorical_cols):
            for j, c2 in enumerate(categorical_cols):
                if j > i:
                    suggestions.append({"name": "Grouped Bar Chart", "for_cols": f"{c1} & {c2}", "type": "Bivariate Categorical-Categorical", "reason": f"Counts of '{c1}' grouped by '{c2}'.", "required_cols_specific": [c1, c2]})
                    suggestions.append({"name": "Heatmap (Counts)", "for_cols": f"{c1} & {c2}", "type": "Bivariate Categorical-Categorical", "reason": f"Co-occurrence frequency of '{c1}' & '{c2}'.", "required_cols_specific": [c1, c2]})

    # Rule: TIME SERIES (DATETIME vs NUMERICAL)
    if (("two" in ua_count or "2" in ua_count) or "time" in ua_types or "trend" in ua_msg) and \
       datetime_cols and numerical_cols:
        for dt_col in datetime_cols:
            for num_col in numerical_cols:
                suggestions.append({"name": "Line Chart", "for_cols": f"{num_col} over {dt_col}", "type": "Time Series", "reason": f"Trend of '{num_col}' over '{dt_col}'.", "required_cols_specific": [dt_col, num_col]})
                suggestions.append({"name": "Area Chart", "for_cols": f"{num_col} over {dt_col}", "type": "Time Series", "reason": f"Cumulative trend of '{num_col}' over '{dt_col}'.", "required_cols_specific": [dt_col, num_col]})
    ...
    if ("more" in ua_count or "mult" in ua_count or "pair" in ua_msg or "heat" in ua_msg or "para" in ua_msg) or \
       ((ua_count not in ["one", "1", "two", "2"]) and (len(numerical_cols) > 2 or len(categorical_cols) > 2)):
        if len(numerical_cols) >= 3:
            suggestions.append({"name": "Pair Plot", "type": "Multivariate", "reason": "Pairwise relationships (numerical).", "required_cols_specific": numerical_cols[:min(4, len(numerical_cols))]})
            suggestions.append({"name": "Correlation Heatmap", "type": "Multivariate", "reason": "Correlation matrix (numerical).", "required_cols_specific": numerical_cols})
        if len(numerical_cols) >= 3: # Can be same condition as above
            suggestions.append({"name": "Parallel Coordinates Plot", "type": "Multivariate", "reason": "Compare multiple numerical variables.", "required_cols_specific": numerical_cols[:min(6, len(numerical_cols))]})

    # --- Deduplication Logic (Multi-line structure for syntax safety) ---
    final_suggestions_dict = {}
    suggestions_order = []
    for s in suggestions:
        req_cols = s.get("required_cols_specific", [])
        s_key_cols_str = "_".join(sorted(req_cols)) if req_cols else ""
        s_key = f"{s['name']}_{s_key_cols_str}" # Construct the key
...
def validate_columns_for_chart(chart_type: str, columns: List[str], df: pd.DataFrame) -> Optional[str]:
    ...
    col_types = get_simplified_column_types(df[columns]) # Validate on the specific columns being used
    ...
    num_distributable_numeric = sum(1 for typ in col_types.values() if typ in ['numerical', 'categorical_numeric'])
    num_datetime = sum(1 for typ in col_types.values() if typ == 'datetime')
    num_selected = len(columns)

    requirements = {
        "Histogram": {'exact_cols': 1, 'distributable_numeric': 1}, "Box Plot": {'exact_cols': 1, 'distributable_numeric': 1},
        "Density Plot": {'exact_cols': 1, 'distributable_numeric': 1}, "Bar Chart (Counts)": {'exact_cols': 1, 'categorical': 1},
        "Pie Chart": {'exact_cols': 1, 'categorical': 1}, "Scatter Plot": {'exact_cols': 2, 'numerical': 2},
        "Line Chart": {'exact_cols': 2, 'numerical': (1, 2)}, # Allow 1 or 2 numerical (e.g. value vs index, or value vs value)
        "Box Plots (by Category)": {'exact_cols': 2, 'categorical': 1, 'distributable_numeric': 1},
        "Violin Plots (by Category)": {'exact_cols': 2, 'categorical': 1, 'distributable_numeric': 1},
        ...
        "Area Chart": {'exact_cols': 2, 'numerical': (1,2)}, "Pair Plot": {'min_cols': 3, 'numerical': 3},
        "Correlation Heatmap": {'min_cols': 2, 'numerical': 2}, "Parallel Coordinates Plot": {'min_cols': 3, 'numerical': 3}
    }
    req = {} # Initialize req
    if chart_type == "Bar Chart":
         if num_selected == 1: req = requirements.get("Bar Chart (Counts)",{}); req['exact_cols']=1 # Default to counts
         elif num_selected == 2:
             ...
             else: return "Bar Chart with 2 columns needs either 2 categorical columns, or 1 categorical and 1 numerical column."
         else: return "Bar Chart needs 1 or 2 columns."
    elif chart_type in requirements:
        req = requirements[chart_type]
    else: # Unknown chart type, skip further validation for it
        return None

    # --- Perform checks based on 'req' ---
    if 'exact_cols' in req and num_selected != req['exact_cols']: return f"{chart_type} needs exactly {req['exact_cols']} column(s), you chose {num_selected}."
    if 'min_cols' in req and num_selected < req['min_cols']: return f"{chart_type} needs at least {req['min_cols']} columns, you chose {num_selected}."
    err_msg_parts = []
    if 'numerical' in req:
        target_num = req['numerical']
    ...
    if 'distributable_numeric' in req and num_distributable_numeric < req['distributable_numeric']: err_msg_parts.append(f"{req['distributable_numeric']} numerical/rating-like (found {num_distributable_numeric})")
    if 'datetime' in req and num_datetime < req['datetime']: err_msg_parts.append(f"{req['datetime']} datetime (found {num_datetime})")

    if err_msg_parts: return f"{chart_type} needs specific column types: {'; '.join(err_msg_parts)}."
    return None
...
def generate_plot_and_get_uri(filepath, chart_type, columns):
    ...
    try:
        ...
        # Validate using a copy of the subset for accurate type checking for those columns
        validation_error = validate_columns_for_chart(chart_type, columns, df_full[columns].copy())
        if validation_error:
        ...
        return None, f"Error reading or validating data: {str(e)[:100]}"

    img = io.BytesIO()
    plt.figure(figsize=(7.5, 5)) # Standard figure size
    plt.style.use('seaborn-v0_8-whitegrid') # Consistent style
    original_chart_type = chart_type
    plot_title_detail = ""
    mapped_chart_type = chart_type # Initialize mapped_chart_type
    plot_columns = list(columns)   # Ensure plot_columns is a mutable list copy

    try:
        print(f"Attempting to generate plot: {original_chart_type} with columns: {plot_columns}")
        col_types_specific = get_simplified_column_types(df_plot)

        # --- Handle Generic "Bar Chart" Request INTELLIGENTLY ---
        if original_chart_type == "Bar Chart": # Use original_chart_type for mapping decision
            if len(plot_columns) == 1 and plot_columns[0] in col_types_specific and \
               col_types_specific[plot_columns[0]] in ['categorical', 'categorical_numeric']:
                mapped_chart_type = "Bar Chart (Counts)"
                plot_title_detail = f" for {plot_columns[0]}"
            elif len(plot_columns) == 2:
                # Determine order for aggregated bar chart if applicable
                cat_cols_present = [c for c in plot_columns if col_types_specific.get(c) in ['categorical', 'categorical_numeric']]
                num_cols_present = [c for c in plot_columns if col_types_specific.get(c) == 'numerical']
                if len(cat_cols_present) == 1 and len(num_cols_present) == 1:
                    mapped_chart_type = "Bar Chart (Aggregated)"
                    # Ensure order is [categorical_col, numerical_col] for plotting function
                    plot_columns = [cat_cols_present[0], num_cols_present[0]]
                    plot_title_detail = f" of {num_cols_present[0]} by {cat_cols_present[0]}"
                elif len(cat_cols_present) == 2:
                    mapped_chart_type = "Grouped Bar Chart"
                    plot_title_detail = f" for {plot_columns[0]} by {plot_columns[1]}"
                else: # Should have been caught by validation
                    raise ValueError("Bar Chart with 2 columns has incompatible types.")
            else: # Should have been caught by validation
                raise ValueError("Bar Chart needs 1 or 2 columns.")
            print(f"--> Handling '{original_chart_type}' as '{mapped_chart_type}' with columns {plot_columns}")
        # --- End Handling Generic "Bar Chart" ---

        plot_title = f"{mapped_chart_type}{plot_title_detail}" # Default title

        # --- Plotting Logic (use mapped_chart_type and plot_columns) ---
        if mapped_chart_type == "Histogram": sns.histplot(data=df_plot, x=plot_columns[0], kde=True); plot_title = f"Histogram of {plot_columns[0]}"
            ...
            df_to_plot = df_plot.copy(); sort_col = plot_columns[0] # Assume first col is x-axis
            try: # Attempt to convert to datetime if not already, then sort
            ...
            except: pass # If conversion or sorting fails, plot as is
            # If x-axis is numeric (and not datetime), sort numerically
            if pd.api.types.is_numeric_dtype(df_to_plot[sort_col]) and not pd.api.types.is_datetime64_any_dtype(df_to_plot[sort_col]):
                df_to_plot = df_to_plot.sort_values(by=sort_col)
        ...
        elif mapped_chart_type == "Bar Chart (Aggregated)":
            cat_col, num_col = plot_columns[0], plot_columns[1] # Uses reordered plot_columns
            agg_data = df_plot.groupby(cat_col)[num_col].mean().nlargest(20)
            sns.barplot(x=agg_data.index, y=agg_data.values); plot_title = f"Mean of {num_col} by {cat_col}"; plt.xlabel(cat_col); plt.ylabel(f"Mean of {num_col}"); plt.xticks(rotation=65, ha='right', fontsize=9)
        elif mapped_chart_type == "Grouped Bar Chart":
            col1_top_categories = df_plot[plot_columns[0]].value_counts().nlargest(10).index
            col2_top_categories = df_plot[plot_columns[1]].value_counts().nlargest(5).index
            df_filtered = df_plot[df_plot[plot_columns[0]].isin(col1_top_categories) & df_plot[plot_columns[1]].isin(col2_top_categories)]
            sns.countplot(data=df_filtered, x=plot_columns[0], hue=plot_columns[1]); plot_title = f"Counts: {plot_columns[0]} by {plot_columns[1]}"; plt.xlabel(plot_columns[0]); plt.ylabel("Count"); plt.xticks(rotation=65, ha='right', fontsize=9); plt.legend(title=plot_columns[1], fontsize='x-small', title_fontsize='small', bbox_to_anchor=(1.02, 1), loc='upper left')
        # Add other specific plot types here as needed
        else:
            raise NotImplementedError(f"Plot type '{mapped_chart_type}' is not explicitly implemented.")

        plt.title(plot_title, fontsize=12)
        plt.tight_layout(pad=1.0) # Adjust padding
        plt.savefig(img, format='png', bbox_inches='tight') # Use bbox_inches for tight layout in save
        plt.close() # Close the figure to free memory
        img.seek(0)
        plot_url = base64.b64encode(img.getvalue()).decode('utf8')
        print(f"Successfully generated plot: {original_chart_type} (as {mapped_chart_type})")
...
def get_response():
    ...
    if current_viz_state == 'asking_variable_types':
        ...
        elif 'suggestions' not in response_data: # Ensure response_data has suggestions if only bot_reply was set
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            df = pd.read_csv(filepath) if filename.endswith(".csv") else pd.read_excel(filepath); session['df_columns'] = list(df.columns)
