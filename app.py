        ...
        try:
            ...
            if is_numeric_dtype(dtype):
                ...
                simplified_types[col] = 'datetime'
                 ...
                 elif unique_count < non_null_count * 0.8 and unique_count < 150: simplified_types[col] = 'categorical'
...
def suggest_charts_based_on_answers(user_answers, df_sample):
    ...
    if ("one" in ua_var_count_str or "1" in ua_var_count_str) and \
       ...
       ("numer" in ua_var_types or "categor" in ua_var_types or "any" in ua_var_types or not ua_var_types) and distributable_numeric_cols:
        for col in distributable_numeric_cols:
            ...
            suggestions.append({"name": "Box Plot", "for_col": col, "type": "Univariate Numerical", "reason": f"Summarizes '{col}'.", "required_cols_specific": [col]})
       ...
       ("mix" in ua_var_types or "categor" in ua_var_types or "numer" in ua_var_types or "any" in ua_var_types or not ua_var_types) and distributable_numeric_cols and categorical_cols:
        for num_col in distributable_numeric_cols:
            for cat_col in categorical_cols:
                ...
                suggestions.append({"name": "Box Plots (by Category)", "for_cols": f"{num_col} by {cat_col}", "type": "Bivariate Numerical-Categorical", "reason": f"Compares distribution of '{num_col}' across '{cat_col}'.", "required_cols_specific": [cat_col, num_col]})
    ...
    for s in suggestions:
        ...
        if s_key not in final_suggestions_dict:
            ...
            suggestions_order.append(s_key) # Preserve order somewhat
    final_suggestions = [final_suggestions_dict[key] for key in suggestions_order] # Reconstruct in rough order
...
def validate_columns_for_chart(chart_type: str, columns: List[str], df: pd.DataFrame) -> Optional[str]:
    ...
    requirements = {
        ...
        "Line Chart": {'exact_cols': 2, 'numerical': (1, 2)}, "Box Plots (by Category)": {'exact_cols': 2, 'categorical': 1, 'distributable_numeric': 1},
    ...
    # Add generic Bar Chart validation based on columns provided
    if chart_type == "Bar Chart":
         if num_selected == 1: req = requirements["Bar Chart (Counts)"]; req['exact_cols'] = 1
         elif num_selected == 2:
             if num_categorical == 2: req = requirements["Grouped Bar Chart"]; req['exact_cols'] = 2
             elif num_categorical == 1 and num_numerical == 1: req = requirements["Bar Chart (Aggregated)"]; req['exact_cols'] = 2
             else: return "Bar Chart with 2 cols needs either 2 categorical or 1 categorical & 1 numerical."
    ...
    if 'numerical' in req:
        if isinstance(req['numerical'], int) and num_numerical < req['numerical']: err_msg += f" {req['numerical']} numerical (found {num_numerical})."; type_error=True
        elif isinstance(req['numerical'], tuple) and not (req['numerical'][0] <= num_numerical <= req['numerical'][1]): err_msg += f" {req['numerical'][0]}-{req['numerical'][1]} numerical (found {num_numerical})."; type_error=True
    if 'categorical' in req:
        if isinstance(req['categorical'], int) and num_categorical < req['categorical']: err_msg += f" {req['categorical']} categorical (found {num_categorical})."; type_error=True
    if 'distributable_numeric' in req:
         if isinstance(req['distributable_numeric'], int) and num_distributable_numeric < req['distributable_numeric']: err_msg += f" {req['distributable_numeric']} numerical/rating-like (found {num_distributable_numeric})."; type_error=True
    if 'datetime' in req:
         if isinstance(req['datetime'], int) and num_datetime < req['datetime']: err_msg += f" {req['datetime']} datetime (found {num_datetime})."; type_error=True
...
# --- Plotting Function (Includes Generic Bar Chart Handling) ---
def generate_plot_and_get_uri(filepath, chart_type, columns):
    ...
    try:
        ...
        if chart_type == "Bar Chart":
            if len(columns) == 1 and columns[0] in col_types_specific and col_types_specific[columns[0]] in ['categorical', 'categorical_numeric']:
                 chart_type = "Bar Chart (Counts)"; plot_title_detail = f" for {columns[0]}"
            elif len(columns) == 2:
                 cat_cols = [c for c in columns if col_types_specific.get(c) in ['categorical', 'categorical_numeric']]
                 num_cols = [c for c in columns if col_types_specific.get(c) == 'numerical']
                 if len(cat_cols) == 1 and len(num_cols) == 1: chart_type = "Bar Chart (Aggregated)"; columns = [cat_cols[0], num_cols[0]]; plot_title_detail = f" for {num_cols[0]} by {cat_cols[0]}"
                 elif len(cat_cols) == 2: chart_type = "Grouped Bar Chart"; plot_title_detail = f" for {columns[0]} by {columns[1]}"
                 else: raise ValueError("Cannot determine Bar Chart type.")
            else: raise ValueError("Bar Chart needs 1 or 2 columns.")
            print(f"--> Handling '{original_chart_type}' as '{chart_type}'")
        # --- End Handling Generic "Bar Chart" ---

        # --- Plotting Logic ---
        plot_title = f"{chart_type}{plot_title_detail}" # Default title
        ...
        # --- END CORRECTION ---
            ...
            sns.lineplot(data=df_to_plot, x=columns[0], y=columns[1]); plot_title = f"Line: {columns[1]} over {columns[0]}"; plt.xlabel(columns[0]); plt.ylabel(columns[1]); plt.xticks(rotation=45, ha='right', fontsize=9)
             ...
             df_plot_filtered = df_plot[df_plot[columns[0]].isin(col1_top_cats) & df_plot[columns[1]].isin(col2_top_cats)]
             sns.countplot(data=df_plot_filtered, x=columns[0], hue=columns[1]); plot_title = f"Counts: {columns[0]} grouped by {columns[1]}" ; plt.xlabel(columns[0]); plt.ylabel("Count"); plt.xticks(rotation=65, ha='right', fontsize=9); plt.legend(title=columns[1], fontsize='x-small', title_fontsize='small', bbox_to_anchor=(1.02, 1), loc='upper left')
        else: raise NotImplementedError(f"Plot type '{chart_type}' is not explicitly implemented.")
        ...
        print(f"Successfully generated plot: {original_chart_type} (as {chart_type})")
        return f"data:image/png;base64,{plot_url}", None

    except Exception as e:
        error_info = f"{type(e).__name__}: {str(e)}"
        print(f"!!! Error during plot generation execution for '{chart_type}' with {columns}: {error_info}")
...
def home():
    ...
    keys_to_clear = ['visualization_questions_state', 'uploaded_filepath', 'uploaded_filename',
                     'df_columns', 'user_answer_variable_types', 'user_answer_visualization_message',
                     'user_answer_variable_count', 'chart_suggestions_list', 'selected_chart_for_plotting',
                     'plotting_columns', 'manual_columns_selected', 'last_suggestions']
...
def get_response():
    ...
    # --- Handle "Suggest another chart" explicitly first ---
    if "suggest another chart" in user_input.lower():
        if session.get('uploaded_filepath') and session.get('user_answer_variable_count') is not None: # Check if context exists
            session['visualization_questions_state'] = 'visualization_info_gathered'
            bot_reply = "Okay, let's look for other visualizations. How should we proceed?"
            ...
            return jsonify(response_data) # Return immediately
        else:
             bot_reply = "Let's start over. Please upload your data file."
             home() # Clear session fully
             response_data = {"suggestions": ["Upload Data", "Help"]}
             # Let it fall through to the end packaging
    ...
    if current_viz_state == 'asking_variable_types':
        ...
        columns_reminder = "" # Shorter reminder
        ...
        if user_choice_str == "Pick columns manually":
            ...
            if required_cols_specific:
                ...
                if validation_msg is None:
                    bot_reply += f"Suggest using: <strong>{cols_to_use_str}</strong>. Plot?"; session['plotting_columns'] = required_cols_specific; response_data = {"suggestions": [f"Yes, plot {chart_name}", "Choose other columns", "Back to chart list"]}; session['visualization_questions_state'] = 'confirm_plot_details'
        ...
        elif "list all columns" in user_input.lower(): bot_reply = f"Available: {', '.join(df_columns)}.<br>Which for {chart_name}?"; response_data = {"suggestions": [f"Use: {col}" for col in df_columns[:2]]}
